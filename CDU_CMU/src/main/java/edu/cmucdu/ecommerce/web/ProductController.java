...
public class ProductController {
    ...
    public String show(@PathVariable("id") Long id, Model uiModel,HttpServletRequest httpRequest) {
    ...
    public String list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel,HttpServletRequest httpRequest) {
		
    	
		if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            Set<Product> p = (Set<Product>) productDao.findAll(new org.springframework.data.domain.PageRequest(firstResult / sizeNo, sizeNo));
            ((Product) p).setLocale(WebUtil.getLocaleEnum(httpRequest)); 
            uiModel.addAttribute("products", ((Page<Product>) p).getContent());
